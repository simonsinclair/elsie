...
export const MealPlan = observer(({ mealPlan, onRemoveClick }: Properties) => {
  ...
  useEffect(() => {
    ...
    return dropTargetForElements({
      ...
      onDrop: action(({ source }) => {
        const meal = source.data.meal;
        if (!(meal instanceof MealModel)) {
          throw new TypeError('`meal` is not an instance of Meal');
        }
